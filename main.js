            ...
            if (message.op == 1) {
                ...
                if (Username) {
                    if ("Channel" in message) {
                        if (Channels.find(v => v.get("Name") == message.Channel && (!v.has("ValidTokens") || v.get("ValidTokens").get(Username) == UToken))) {
                            wsChannel.get("Users").delete(Username)
                ...
                if (Username) {
                    ValidChannels = Channels.filter(v => !v.get("ValidTokens") || v.get("ValidTokens").get(Username) == UToken).map(map => map.get("Name")).join(", ")
...
function broadcast(data, channel) {
    channel.get("Users").forEach((client) => {
